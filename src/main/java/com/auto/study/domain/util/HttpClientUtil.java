...
public class HttpClientUtil implements Serializable {
     ...
     * 发起get请求，返回 response
     */
    public CloseableHttpResponse sendPostRequestForResponseWithParam(String url, String json) throws Exception {
        HttpPost httpPost = new HttpPost(url);
        setDefaultHeaders(httpPost);
     ...
     * 发起get请求，返回 response
     */
    public String sendPostRequestForHtmlWithParam(String url, String jsonParam) throws Exception {
        CloseableHttpResponse response = sendPostRequestForResponseWithParam(url, jsonParam);
        String html = responseToString(response);
        closeResponseAndIn(null, response);
        return html;
    }
